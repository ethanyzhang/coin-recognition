...
void NeuralNet::loadTrainingParameter(int input_count,
									 int output_count,
...
void NeuralNet::loadNetwork(std::vector<int>& layer_size,
				 			double p_bias,
				 			std::string activation_type,
	...
	this->bias = p_bias;
	this->activationType = activation_type;
	this->netWeights = net_weights;

	this->hiddenLayers.resize(layer_size.size() - 2);

	this->createNetwork();
	this->initializeTrained();
}
...
void NeuralNet::initializeTrained() {
	int idx = 0;
	for (int i = 0; i < this->hiddenLayers.size(); i++) {
		for (int j = 0; j < this->hiddenLayers[i].neuronCount; j++) {
			for (int k = 0; k < this->hiddenLayers[i].neurons[j].inputCount; k++) {
				this->hiddenLayers[i].neurons[j].weights[k] = this->netWeights[idx];
				idx++;
			}
		}
	}
	...
	for (int i = 0; i < this->outputLayer.neuronCount; i++) {
		for (int j = 0; j < this->outputLayer.neurons[i].inputCount; j++) {
			this->outputLayer.neurons[i].weights[j] = this->netWeights[idx];
			idx++;
		}
	}
}
...
bool NeuralNet::saveNetWeights() {
	for (int i = 0; i < this->hiddenLayers.size(); i++) {
		for (int j = 0; j < this->hiddenLayers[i].neuronCount; j++) {
			for (int k = 0; k < this->hiddenLayers[i].neurons[j].inputCount; k++) {
				this->netWeights.push_back(this->hiddenLayers[i].neurons[j].weights[k]);
			}
		}
	}
	...
	for (int i = 0; i < this->outputLayer.neuronCount; i++) {
		for (int j = 0; j < this->outputLayer.neurons[i].inputCount; j++) {
			this->netWeights.push_back(this->outputLayer.neurons[i].weights[j]);
		}
	}
...
bool NeuralNet::writeNetParameters() {
	std::string netWeightsPath = "../data/NETPARAMETERS.txt";
	std::ofstream out(netWeightsPath, ios::trunc);

	for (int i = 0; i < this->layerSize.size(); i++) {
		out << this->layerSize[i] << " ";
	}
	out << "\n";

	out << this->bias << "\n";
	out << this->activationType << "\n";

	for (int i = 0; i < this->netWeights.size(); i++) {
		out << this->netWeights[i] << " ";
	}
	out << "\n";
